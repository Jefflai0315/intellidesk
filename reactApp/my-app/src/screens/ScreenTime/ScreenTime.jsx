import React from "react";
import { BotBar_Home } from "../../components/BotBar_Home";
import "./style.css";
import { Link } from 'react-router-dom';

function ScreenTime() {
  return (
    <div className="desk-time">
      <div className="overlap-wrapper">
        <div className="overlap">
          <div className="background" />
          <div className="title">
            <div className="overlap-group">
              <div className="text-wrapper">Intellidesk</div>
              <img className="line" alt="Line" src="https://c.animaapp.com/QHWYwOlz/img/line-1.svg"/>
            </div>
          </div>
          <BotBar_Home className="bot-bar-home-instance-node" />
          <div className="bot-bar-home">
            <div className="div">
              <div className="group">
                <div className="text-wrapper-2">More</div>
                <div className="group-2">
                  <div className="ellipse-wrapper">
                    <div className="ellipse" />
                  </div>
                  <div className="div-wrapper">
                    <div className="ellipse" />
                  </div>
                  <div className="group-3">
                    <div className="ellipse" />
                  </div>
                </div>
              </div>
              <div className="text-wrapper-3">My Timers</div>
              <img
                className="asset"
                alt="Asset"
                src="https://c.animaapp.com/QHWYwOlz/img/asset-7-720x-1@2x.png"
              />
              <div className="overlap-group-wrapper">
                <div className="overlap-2">
                  <div className="text-wrapper-4">My Day</div>
                  <div className="group-4">
                    <div className="overlap-group-2">
                      <div className="rectangle" />
                      <div className="rectangle-2" />
                    </div>
                  </div>
                </div>
                <div className="group-5">
                  <div className="text-wrapper-5">My Table</div>
                  <div className="group-6">
                    <div className="rectangle-3" />
                    <img
                      className="img"
                      alt="Group"
                      src="https://c.animaapp.com/QHWYwOlz/img/group-29-1@2x.png"
                    />
                  </div>
                </div>
                <div className="group-7">
                  <div className="text-wrapper-6">My Goals</div>
                </div>
                <img
                  className="asset-x"
                  alt="Asset"
                  src="https://c.animaapp.com/QHWYwOlz/img/asset-15-720x-1@2x.png"
                />
              </div>
            </div>
            <div className="rectangle-4" />
            <div className="navbar">
              <div className="text-wrapper-7">24/9</div>
              <div className="text-wrapper-8">25/9</div>
              <div className="text-wrapper-9">26/9</div>
              <div className="text-wrapper-10">27/9</div>
              <div className="text-wrapper-11">28/9</div>
              <div className="text-wrapper-12">29/9</div>
              <div className="text-wrapper-13">30/9</div>
            </div>
            <div className="grid-lines">
              <img
                className="line-2"
                alt="Line"
                src="https://c.animaapp.com/QHWYwOlz/img/line-2.svg"
              />
              <img
                className="line-3"
                alt="Line"
                src="https://c.animaapp.com/QHWYwOlz/img/line-2.svg"
              />
              <img
                className="line-4"
                alt="Line"
                src="https://c.animaapp.com/QHWYwOlz/img/line-2.svg"
              />
              <img
                className="line-5"
                alt="Line"
                src="https://c.animaapp.com/QHWYwOlz/img/line-2.svg"
              />
              <img
                className="line-6"
                alt="Line"
                src="https://c.animaapp.com/QHWYwOlz/img/line-2.svg"
              />
              <img
                className="line-7"
                alt="Line"
                src="https://c.animaapp.com/QHWYwOlz/img/line-2.svg"
              />
              <img
                className="line-8"
                alt="Line"
                src="https://c.animaapp.com/QHWYwOlz/img/line-2.svg"
              />
              <img
                className="line-9"
                alt="Line"
                src="https://c.animaapp.com/QHWYwOlz/img/line-2.svg"
              />
            </div>
            <div className="group-8">
              <div className="overlap-3">
                <div className="bar-chart" />
                <div className="overlap-group-3">
                  <div className="rectangle" />
                  <div className="rectangle-2" />
                  <div className="rectangle-3" />
                  <div className="rectangle-4" />
                </div>
              </div>
              <div className="bar-chart-2" />
              <div className="overlap-group-3">
                <div className="rectangle-9" />
                <div className="rectangle-10" />
                <div className="rectangle-11" />
              </div>
              <div className="rectangle-12" />
            </div>
            <div className="bar-chart-3" />
            <div className="overlap-group-3" />
            <div className="rectangle-13" />
            <div className="rectangle-14" />
            <div className="rectangle-15" />
          </div>
          <div className="group-10">
            <div className="rectangle-16" />
            <div className="rectangle-17" />
            <div className="rectangle-18" />
          </div>
          <div className="group-11">
            <div className="rectangle-19" />
            <div className="overlap-4">
              <div className="rectangle-20" />
              <div className="rectangle-21" />
            </div>
            <div className="rectangle-22" />
          </div>
          <div className="overlap-5">
            <div className="group-12">
              <div className="rectangle-23" />
              <div className="overlap-6">
                <div className="rectangle-24" />
                <div className="rectangle-25" />
              </div>
            </div>
            <div className="bar-chart-4">
              <div className="overlap-7">
                <div className="rectangle-13" />
                <div className="rectangle-26" />
              </div>
              <div className="rectangle-wrapper">
                <div className="rectangle-27" />
              </div>
            </div>
          </div>
          <div className="text-wrapper-14">Desk Time</div>
          <div className="desk-time-summary">
            <div className="average-DT">
              <div className="text-wrapper-15">Average</div>
              <div className="overlap-group-4">
                <div className="text-wrapper-16">7.8</div>
                <div className="text-wrapper-17">hrs/day</div>
              </div>
            </div>
            <div className="overlap-8">
              <div className="stats">
                <div className="overlap-9">
                  <div className="text-wrapper-18">hrs</div>
                  <div className="text-wrapper-19">20</div>
                </div>
                <div className="overlap-10">
                  <div className="text-wrapper-20">hrs</div>
                  <div className="text-wrapper-21">35</div>
                </div>
              </div>
              <div className="legend">
                <div className="ellipse-2" />
                <div className="ellipse-3" />
                <div className="text-wrapper-22">Standing</div>
                <div className="text-wrapper-23">Sitting</div>
              </div>
            </div>
          </div>
          <div className="date-line">
            <div className="group-13">
              <div className="text-wrapper-24">00:00</div>
              <div className="text-wrapper-25">12:00</div>
              <div className="text-wrapper-26">23:59</div>
            </div>
            <div className="group-14">
              <div className="ellipse-4" />
              <div className="ellipse-5" />
              <div className="ellipse-6" />
              <div className="ellipse-7" />
              <div className="ellipse-8" />
              <div className="ellipse-9" />
              <div className="ellipse-10" />
            </div>
          </div>
          <div className="group-15">
            <div className="navbar-wrapper">
              <div className="div-2">
                <div className="text-wrapper-27">1d</div>
                <div className="text-wrapper-28">4w</div>
                <div className="text-wrapper-29">7d</div>
                <div className="text-wrapper-30">1y</div>
              </div>
            </div>
            <div className="text-wrapper-31">24/9 - 30/9</div>
          </div>
          <div className="group-16">
            <div className="div-2">
              <div className="text-wrapper-32">24</div>
              <div className="text-wrapper-33">28</div>
              <div className="overlap-group-5">
                <div className="text-wrapper-34">26</div>
              </div>
              <div className="text-wrapper-30">30</div>
              <div className="text-wrapper-35">25</div>
              <div className="text-wrapper-36">27</div>
              <div className="text-wrapper-37">29</div>
            </div>
          </div>
          <div className="group-17">
            <div className="average-SD">
              <div className="overlap-group-6">
                <div className="text-wrapper-38">Longest Standing</div>
                <div className="text-wrapper-39">2 hours</div>
              </div>
            </div>
            <div className="closest">
              <div className="overlap-11">
                <div className="text-wrapper-40">3hrs 13mins</div>
                <div className="text-wrapper-41">Longest Sitting</div>
              </div>
            </div>
            <div className="closest-2">
              <div className="overlap-12">
                <div className="text-wrapper-42">4hrs 10mins</div>
                <div className="text-wrapper-43">Total Sitting</div>
              </div>
            </div>
            <div className="closest-3">
              <div className="overlap-13">
                <div className="text-wrapper-42">1hrs 40mins</div>
                <div className="text-wrapper-44">Total Break</div>
              </div>
            </div>
            <div className="closest-4">
              <div className="overlap-14">
                <div className="text-wrapper-45"> 59mins</div>
                <div className="text-wrapper-46">Longest Break</div>
              </div>
            </div>
            <div className="closest-5">
              <div className="overlap-14">
                <div className="text-wrapper-45">3 hours</div>
                <div className="text-wrapper-46">Total Standing</div>
              </div>
            </div>
          </div>
          <img
            className="frame"
            alt="Frame"
            src="frame-1.svg"
          />
          {/* <BotBar_Home className="bot-bar-home-instance" /> */}
        </div>
      </div>
    </div>
  );
}

export default ScreenTime;
